...
#[tauri::command]
pub async fn handle_drop(app: tauri::AppHandle, state: tauri::State<'_, AppState>, path: String) -> Result<(), String> {
    if !state.archive_settings.lock().unwrap().enabled {
        let _ = app.emit(EVENT_ARCHIVE_RESULT, serde_json::json!({ "ok": false, "error": "Archive feature is disabled" }));
        return Ok(());
    }
    let tool = state
        .tools
    ...
    let result = tool.execute(serde_json::json!({ "action": action, "path": path })).await;
